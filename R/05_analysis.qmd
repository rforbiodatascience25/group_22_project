...
# calculate the side effect rates used for correlation plot. we need to 
# extract the list of frequencies and calculate the average over that
...
correlation <- cor(side_effect_rates$n_reports, side_effect_rates$avg_rate, 
                   use = "complete.obs")
print(str_c("Correlation between number of reports and average frequency rate:", round(correlation, 3)))
...
plot_correlation <- side_effect_rates |>
  ...
  labs(
    ...
    subtitle = str_c("Correlation coefficient:", round(correlation, 3)),
    x = "Number of Reports (Drug + Side Effect Pairs)",
    y = "Average Frequency Rate (%)"
  ) +
  theme_minimal()
ggsave(str_c(results_path,"Analysis1_reports_vs_freq.png"), plot = last_plot(), 
...
```{r}
#| warning: false
# We only filter for the top 20 indications and top 20 side effects to 
# make this viewable top 20 indications (treatment target)
top_indications <- df |>
...
ggsave(str_c(results_path,"Analysis1_co_occ_indic.png"), plot = last_plot(), 
       width = 8, height = 6, dpi = 300)
print(plot_matrix)
```
...
ggsave(str_c(results_path,"Analysis1_co_occ_indic_freq.png"), 
...
# In this snippet, we research if we can see the receptor that the 
# drug targets is closely linked with specific side effects. 
# Again we just take top 20
top_targets <- df |>
...
ggsave(
  str_c(results_path, "Analysis1_co_occ_targets.png"),
...
drug_targets_se <- df |>
  mutate(uniprot_names = str_split(uniprot_names, ";")) |> 
  unnest(uniprot_names) |> 
  mutate(uniprot_names = trimws(uniprot_names)) |> 
  filter(uniprot_names != "") |> 
...
plot_targets_vs_se <- drug_targets_se |>
  ...
  labs(
    ...
    subtitle = str_c("Correlation:", round(cor_targets, 3)),
    x = "Number of Protein Targets",
    y = "Number of Unique Side Effects"
  ) +
  theme_minimal()

ggsave(str_c(results_path,"Analysis1_targets_se_correl.png"), 
       plot = last_plot(), 
       width = 8, height = 6, dpi = 300)
...
cor_indications <- cor(drug_indications_se$n_indications, 
                       drug_indications_se$n_side_effects,
                       use = "complete.obs")
...
plot_indications_vs_se <- drug_indications_se |>
  ...
  labs(
    ...
    subtitle = str_c("Correlation:", round(cor_indications, 3)),
    x = "Number of Indications",
    y = "Number of Unique Side Effects"
  ) +
  theme_minimal()

ggsave(str_c(results_path,"Analysis1_indications_se_correl.png"), 
       plot = last_plot(), 
       width = 8, height = 6, dpi = 300)
...
hormone_pattern <- str_c(hormone_targets, collapse="|")
...
met_pattern <- str_c(met_terms, collapse = "|")
