...
## Downloading the SIDER data set
...
```{r}
write_tsv(drug_complete, "../data/sider_full_clean.tsv")
```
...
```{r}
library(readr)
vocabulary <- read_csv("../data/_raw/drugbank/drugbank vocabulary.csv")
structures <- read_csv("../data/_raw/drugbank/structure links.csv")
targets <- read_csv("../data/_raw/drugbank/uniprot links.csv")
...
drugbank_merged <- select(drugbank_merged, -"CAS Number", -"InChIKey", -"Name.x", -"Name.y")

colnames(drugbank_merged)
