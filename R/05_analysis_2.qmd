...
hormonal_drugs <- df |> mutate(hormone_target = str_detect(tolower(UniProt_Names), hormone_pattern))
...
hormonal_drugs_endo_targets <- hormonal_drugs |> mutate(endocrine_se = str_detect(side_effect_PT, paste(endo_terms, collapse="|")))
...
hormonal_drugs_endo_targets |>
  ...
  mutate(
    hormone_target = ifelse(hormone_target, "Hormonal target", "Non-hormonal target")
  ) |>
...
ggsave("../results/Analysis1.2_Endocrine.png", plot = last_plot(), width = 8, height = 6, dpi = 300)
...
sa_receptor_metabolic_se <- metabolic_se |> mutate(sa_target = str_detect(tolower(UniProt_Names), sa_receptor_pattern))
...
sa_receptor_metabolic_se |>
  ...
  mutate(
    sa_target = ifelse(sa_target, "Stress-axis target", "Non-stress-axis target")
  ) |>
...
ggsave("../results/Analysis1.2_Metabolic.png", plot = last_plot(), width = 8, height = 6, dpi = 300)
